...
class Mappress_Pro extends Mappress {
	...
	*/
	function save_post_create($post_ID) {
		...
		$options = Mappress_Options::get();
		$this->create_meta_map($post_ID, $options->metaKey, $options->metaLink, false);
	}
	...
	*/
	function save_post_update($post_ID) {
		...
		$options = Mappress_Options::get();
		$this->create_meta_map($post_ID, $options->metaKey, $options->metaLink, true);
	}
	...
	*/
	function post_meta_update($meta_id, $object_id, $meta_key, $meta_value) {
		...
		if (!$options->metaKey || $meta_key != $options->metaKey)
			return;
		$this->create_meta_map($object_id, $options->metaKey, $options->metaLink, true);
	}
	...
	function get_meta_pois($postid, $meta_key) {
		...
		if ($metapois) {
			foreach((array)$metapois as $metapoi) {
				...
				{	
					// if we are isssuing th geocoding commands too fast google will give us an error so loop
					// with a sleep to try and compensate for this.. total of 6 seconds
					for ($i=1; $i<=3; $i++)
					{
						//if (strchr($result->get_error_message('geocode'),"OVER_QUERY_LIMIT") != FALSE)
						if (is_wp_error($result))
						{
							sleep($i);
							$result = $poi->geocode();
	...
	*/
	function create_meta_map($postid, $meta_key, $meta_link, $update=false, $atts=null) {
	
		$map_tx_name = $this->get_post_mapname_by_taxonomy($postid);
		...
		$map->update(array('pois' => $meta_pois, 'title' => $map_tx_name, 'metaKey' => $meta_key, 'center' => array('lat' => 0, 'lng' => 0)));
		$map->save($postid);
	...
	/**
	* get_post_mapname_by_taxonomy.  Returns a map name based upon the Map taxonomy or untitled if not associated with Map taxonomy.
	*
	* @param mixed $postid
	*/
	function get_post_mapname_by_taxonomy($postid) {
		global $wpdb;

		$terms = get_the_terms( $postid->ID, 'maps' );
		...
		if ( $terms && ! is_wp_error( $terms ) ) 
		{
			...
			/* build an array of the map location : Continent-Country-Region-State */
			foreach ( $terms as $term ) 
			{
				if (strcmp($term->name,"Continent") != 0  && 
					strcmp($term->name,"Country") != 0  && 
					strcmp($term->name,"Region") != 0  &&
					strcmp($term->name,"State") != 0  )
				{
					$map_terms[] = $term->name;
				}
			}
							
			$map_name = join( "-", $map_terms );
			return $map_name;
		}
		else 
			return "untitled";
	...
	*/
	...
	function set_options_pro($input) {
		// Set binary options
	...
	function set_meta_key() {
		...
		foreach ($meta_keys as $meta_key)
			echo "<option " . selected($meta_key, $options->metaKey, false) . " value='$meta_key'>$meta_key</option>";
		echo "</select>";
		
		echo "<br/>";
		echo __("Field for POI Body Links", 'mappress');
		echo ": <select name='mappress_options[metaLink]'>";
		foreach ($meta_keys as $meta_key)
			echo "<option " . selected($meta_key, $options->metaLink, false) . " value='$meta_key'>$meta_key</option>";
		echo "</select>";
		...
		foreach ($meta_keys as $meta_key)
			echo "<option " . selected($meta_key, $options->metaKeyErrors, false) . " value='$meta_key'>$meta_key</option>";
		echo "</select>";

		echo "<br/><input type='checkbox' name='mappress_options[metaSyncSave]' " . checked($options->metaSyncSave, true, false) . "/> ";
		_e("Update map when post is updated", 'mappress');

		echo "<br/><input type='checkbox' name='mappress_options[metaSyncUpdate]' " . checked($options->metaSyncUpdate, true, false) . "/> ";
	...
	function set_poi_list() {
		...
		echo "<input type='checkbox' name='mappress_options[poiList]' " . checked($options->poiList, true, false) . "/> ";
	...
	function set_poi_list_template() {
		...
		echo esc_attr($options->poiListTemplate);
		echo "</textarea>";
	}
	...
	function set_control() {
		...
		echo "<input type='checkbox' name='mappress_options[control]' " . checked($options->control, true, false) . "/> ";
